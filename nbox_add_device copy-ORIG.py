...
class NboxApi:
    ...
    def get_multi_id(self, api_attr, input_obj, input_obj_type, other_obj_type, error):
        ...
        try:
            result[input_obj_type] = (
                operator.attrgetter(api_attr)(self.nb).get(name=input_obj).id
            )
            # If the secondary object (other_obj) is defined also gets that objects ID
            if (
                operator.attrgetter(other_obj_type)(
                    operator.attrgetter(api_attr)(self.nb).get(name=input_obj)
                )
                != None
            ):
                result[other_obj_type] = operator.attrgetter(other_obj_type)(
                    operator.attrgetter(api_attr)(self.nb).get(name=input_obj)
                ).id
            return result
        ...
        # Catch-all for any other error
            ...
            self.rc.print(
                ":x: [red]def {}[/red] using {} [i red]{}[/i red] - {}".format(
                    "get_multi_id",
    ...
    def get_single_id(self, vm_name, api_attr, input_obj, error):
        try:
            ...
            error.append(
                (vm_name, {api_attr.split(".")[1].capitalize()[:-1]: input_obj}, e)
            )
        # Catch-all for any other error
        except Exception as e:
            self.rc.print(
    ...
    def chk_exist(
        ...
        try:
            result = operator.attrgetter(api_attr)(self.nb).get(
                **{input_obj_fltr: input_obj, obj_cntr_fltr: obj_cntr_id}
            )
            return result
        # Catch-all for any other error
        except Exception as e:
            self.rc.print(
                ":x: [red]def {}[/red] using {} [i red]{}[/i red] in {} [i]{}[/i] - {}".format(
                    "chk_exist",
                    api_attr.split(".")[1].capitalize()[:-1],
                    input_obj,
                    obj_cntr_fltr,
...
class CreateDm:
    ...
    def create_vm(self, clstr_site, each_clstr, each_vm, tmp_vm):
        vm = dict(
            name=each_vm["name"],
            clstr_name=each_clstr["name"],
            cluster=clstr_site["clstr"],
            site=clstr_site.get("site", None),
            tenant=tmp_vm.get("tenant", None),
            role=tmp_vm.get("role", None),
            platform=tmp_vm.get("platform", None),
            vcpus=each_vm.get("cpu", None),
            memory=each_vm.get("mem", None),
            disk=each_vm.get("disk", None),
            comments=each_vm.get("comments", ""),
            tags=each_vm.get("tags", []),
        )
        return vm
    ...
    def vm_error(self, vm_name, input_err):
        ...
        self.rc.print(
            ":x: Virtual Machine [i red]{}[/i red] objects may not exist. Failed to get object.id for - {}".format(
                vm_name, str(dict(tmp_err)).replace("{", "").replace("}", "")
            )
        )
    ...
    def engine(self):
        ...
        for each_clstr in self.my_vars["cluster"]:
            ...
            clstr_site = self.nbox.get_multi_id(
            ...
            if len(clstr_err) == 0:
                for each_vm in each_clstr["vm"]:
                    vm_err, intf_err, intf, ip = (
                        [] for i in range(4)
                    )  # Need to be reset each VM iteration
                    tmp_vm = {}
                    for obj, uri in dict(
                        ...
                        if each_vm.get(obj) != None:
                            tmp_vm[obj] = self.nbox.get_single_id(
                                each_vm["name"], uri, each_vm[obj], vm_err
                            )
            ...
            # FAILFAST_CLUSTER: Reports error message if Cluster does not exist
                ...
                for obj_type, name, err in clstr_err:
                    self.rc.print(
                        ":x: {} [i red]{}[/i red] may not exist. Failed to get object.id - {}".format(
                            obj_type, name, err
                        )
                    )
        return all_vm
...
class CreateObject:
    ...
    def crte_upte_stdout(
        ...
        if len(deploy_err) != 0:
            ...
            self.rc.print(
                ":x: Virtual Machine [i red]{}[/i red] {} {} failed - {}".format(
                    dm["vm"]["name"],
                    obj_type,
                    err["deploy_type"],
                    str(err["err"])
            ...
            if (
                vm_rslt["changed"] == False
                and intf_rslt.get("changed", False) == False
                and ip_rslt.get("changed", False) == False
            ):
                self.rc.print(
                    "\u26A0\uFE0F  Virtual Machine [i yellow]{}[/i yellow] already exists with the correct details".format(
                        vm_result[1]
                    )
                )
            else:
                ...
                if vm_rslt["changed"] == True:
                    ...
                    if dm["vm"].get("site") != None:  # As site is not always present
                        dm["vm"]["site"]
                    vm_rslt = dict(
                        details=str(list(dm["vm"].keys()))
                        .replace("[", "")
                        .replace("]", "")
                    )
                    vm_rslt[
                        "details"
                    ] = "[b #000000]attributes:[/b #000000] [i]{}[/i]".format(
                        vm_rslt["details"]
                    )
                ...
                self.rc.print(
                    ":white_heavy_check_mark: Virtual Machine [i green]{}[/i green] {}d with {} {} {}".format(
                        vm_result[1],
                        vm_result[0],
                        vm_rslt.get("details", ""),
    ...
    def create_vm(self, dm, vm_exist):
        ...
        vm_result = self.nbox.obj_create(
            dm["vm"]["name"], "virtualization.virtual_machines", dm["vm"], deploy_err
        )
    ...
    def update_vm(self, dm, vm_exist):
        ...
        if len(dm["vm"]) > 4:
            vm_result = self.nbox.obj_update(
                dm["vm"]["name"], vm_exist, dm["vm"], deploy_err
            )
        else:
            vm_result = [
                "update",
                vm_exist,
                False,
            ]  # Replicates what would be returned by nbox.obj_update
        # STDOUT: Only print message if error or no interfaces defined
        if len(deploy_err) != 0 or len(dm["intf"]) == 0:
            self.crte_upte_stdout("vm", vm_exist, dm, deploy_err, vm_result)
        result = dict(vm_result=vm_result, deploy_err=deploy_err)
    ...
    def crte_upte_intf(self, dm, vm_exist, vm_result, deploy_err):
        ...
        if len(deploy_err) != 0 or len(dm["ip"]) == 0:
            self.crte_upte_stdout(
                "interface", vm_exist, dm, deploy_err, vm_result, intf_result
            )
    ...
    def crte_upte_ip(self, dm, vm_exist, vm_result, intf_result, deploy_err):
        ...
        if len(deploy_err) != 0:
            self.crte_upte_stdout(
                "ip", vm_exist, dm, deploy_err, vm_result, intf_result
            )
        ...
        self.crte_upte_stdout(
            "ip", vm_exist, dm, deploy_err, vm_result, intf_result, ip_result
        )
    ...
    def engine(self, dm):
        ...
        for each_vm in dm:
            vm_exist = self.nbox.chk_exist(
                "virtualization.virtual_machines",
                "name",
                each_vm["vm"]["name"],
                "cluster",
                each_vm["vm"]["cluster"],
                each_vm["vm"]["clstr_name"],
            )
            ...
            if len(each_vm["intf"]) != 0:
                if len(vm_info.get("deploy_err", ["dummy"])) == 0:
                    intf_info = self.crte_upte_intf(
                        each_vm, vm_exist, vm_info["vm_result"], vm_info["deploy_err"]
                    )
