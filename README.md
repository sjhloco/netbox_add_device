...
| Parent | Key | Mandatory | Description |
| ------ | --- | --------- | ----------- |
...
Clone the repository and use uv to create the virtual environment and install the required python packages.

```bash
git clone https://github.com/sjhloco/netbox_add_device.git
cd netbox_add_device

uv sync
```

To get started set the Netbox API URL and token environment variables, SSL checking can be disabled or the SSL CA cert location defined if using HTTPS with a self-signed certificate.
